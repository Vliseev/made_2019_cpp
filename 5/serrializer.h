...
#pragma once
#include <charconv>
#define CHECK_TYPE(x) std::is_same<x, uint64_t>() || std::is_same<x, bool>()
...
class Serializer {
 ...
 private:
    ...
    template <class T>
    Error Process(T& val) {
        typedef typename std::remove_reference<decltype(val)>::type C;
        static_assert(std::is_same<C, bool>() || std::is_same<C, uint64_t>());
...
class Deserializer {
 ...
 private:
    ...
    Error Process(T& val) {
        typedef typename std::remove_reference<decltype(val)>::type C;
        if (std::is_same<C, uint64_t>() || std::is_same<C, bool>()) {
